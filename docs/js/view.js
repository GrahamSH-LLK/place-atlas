...
function renderBackground(collage) {
  ...
  backgroundContext.fillStyle = "rgba(0, 0, 0, 0.4)";
...
function initView() {
  ...
  var closeObjectsListButton = document.getElementById(
  ...
  renderBackground(collage);
  render();

  buildObjectsList(null, null);

  // parse linked collage entry id from link hash
  ...
  function shuffle() {
    //console.log("shuffled collage");
    for (var i = sortedCollage.length - 1; i > 0; i--) {
      var j = Math.floor(Math.random() * (i + 1));
      var temp = sortedCollage[i];
      sortedCollage[i] = sortedCollage[j];
      sortedCollage[j] = temp;
    }
  }
  ...
  function highlightEntryFromUrl() {
    ...
    var entry = collage.filter(function (e) {
      return e.id === id;
    });
    ...
    if (entry.length === 1) {
      ...
      document.title = entry.name + " on the /r/place Collage";
  ...
  function updateHovering(e, tapped) {
    if (!dragging && (!fixed || tapped)) {
      ...
      if (
        ...
        var newHovered = [];
        for (var i = 0; i < collage.length; i++) {
          if (pointIsInPolygon(pos, collage[i].path)) {
            newHovered.push(collage[i]);
          }
        }
  ...
  function buildObjectsList(filter, sort) {
    ...
    if (!sortedCollage) {
      sortedCollage = collage.concat();
      document.getElementById("collageSize").innerHTML =
        "The Collage contains " + sortedCollage.length + " entries.";
    }

    if (filter) {
      sortedCollage = collage.filter(function (value) {
        return (
      ...
      document.getElementById("collageSize").innerHTML =
        "Found " + sortedCollage.length + " entries.";
    } else {
      document.getElementById("collageSize").innerHTML =
        "The Collage contains " + sortedCollage.length + " entries.";
    }
    ...
    renderBackground(sortedCollage);
    render();
      ...
      case "relevant":
        sortFunction = function (a, b) {
          if (
            ...
            if (
              a.name.toLowerCase().indexOf(filter) <
              b.name.toLowerCase().indexOf(filter)
            ) {
              return -1;
            } else if (
              a.name.toLowerCase().indexOf(filter) >
            ...
            if (
              a.description.toLowerCase().indexOf(filter) <
              b.description.toLowerCase().indexOf(filter)
            ) {
              return -1;
            } else if (
              a.description.toLowerCase().indexOf(filter) >
    ...
    if (sortFunction) {
      sortedCollage.sort(sortFunction);
    }

    for (var i = entriesOffset; i < entriesOffset + entriesLimit; i++) {
      if (i >= sortedCollage.length) {
        break;
      }

      var element = createInfoBlock(sortedCollage[i]);

      element.entry = sortedCollage[i];
      ...
      element.addEventListener("mouseleave", function (e) {
        if (!fixed && !dragging) {
          zoomOrigin = [
            previousScaleZoomOrigin[0] * zoom,
            previousScaleZoomOrigin[1] * zoom,
          ];
          scaleZoomOrigin = [
            previousScaleZoomOrigin[0],
            previousScaleZoomOrigin[1],
          ];
          applyView();
          hovered = [];
          updateLines();
    ...
    if (sortedCollage.length > entriesOffset) {
      moreEntriesButton.innerHTML =
        "Show " +
        Math.min(entriesLimit, sortedCollage.length - entriesOffset) +
        " more";
      entriesList.appendChild(moreEntriesButton);
    }
  }
