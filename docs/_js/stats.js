...
for (var q = 0; q < collage.length; q++) {
  var path = collage[q].path;
  ...
  if (collage[q].name == "Companion Cube") {
    var w = collage[q].path[1][0] - collage[q].path[0][0];
    var h = collage[q].path[2][1] - collage[q].path[1][1];
    console.log(w, h, w * h);
...
console.log(
  "There are " +
    ...
    max +
    ", accounting for " +
    (largerThanMax / areas.length) * 100 +
    "% of all entries."
);
console.log(
  "The largest entry has an area of " + areas[areas.length - 1] + " pixels."
);
...
for (var i = 0; i <= Math.ceil(mostCounts / linesDistance / 5) * 5; i++) {
  ...
  ctx.moveTo(
    50,
    ~~(el.height - 50 - i * (linesDistance / mostCounts) * (el.height - 100)) +
      0.5
  );
  ctx.lineTo(
    el.width - 25,
...
for (var i = 0; i <= counts.length; i++) {
  if (i % skip == 0) {
    ...
    ctx.lineTo(~~((i / steps) * (el.width - 125) + 75) + 0.5, y);
    ctx.stroke();
...
console.log("Average area: " + areasSum / collage.length);
...
for (var i = 0; i < topCount; i++) {
  outstring +=
    ...
    1 +
    "|[" +
    collage[collage.length - i - 1].name +
    "](https://draemm.li/various/place-collage/?id=" +
    collage[collage.length - i - 1].id +
    ")|" +
    ~~collage[collage.length - i - 1].area +
    "|" +
    Math.round(collage[collage.length - i - 1].area / 100) / 100 +
    "%\n";
}
...
console.log("Median x: " + collage[~~(collage.length / 2)].center[0]);
...
console.log("Median y: " + collage[~~(collage.length / 2)].center[1]);
